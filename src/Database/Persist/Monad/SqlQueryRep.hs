...
-- | Class of backends of which the queries can be represented in Haskell values
class QueryRepresentable backend where
  -- | The representation of a @backend@ query. The first type argument is the
  -- record, and the second type argument is the return type of the query.
  data QueryRep backend :: * -> * -> *

-- | Class of backends with compatible query representations.
--
-- If @'QueryRepCompatible' sup sub@, then queries against @sup@ can also
-- represented as queries against @sub@ - so @sub@ must be at least as strong
-- as @sup@ at running queries.
--
-- This is the equivalent to Persistent's 'BackendCompatible', but specifically
-- for 'QueryRepresentable' backends.
class QueryRepCompatible sup sub where
  projectQueryRep :: QueryRep sup record a -> QueryRep sub record a

instance QueryRepCompatible backend backend where
  projectQueryRep = id
...
withCompatibleQueryRep
  ...
  => (QueryRep sub record a -> b)
  -> QueryRep sup record a
  -> b
withCompatibleQueryRep = (. projectQueryRep)

instance QueryRepresentable SqlBackend where
  ...
  data QueryRep SqlBackend record a where
...
type SqlQueryRep = QueryRep SqlBackend
